...
namespace Ryujinx.HLE.HOS.Services.Mii.Types
{
    ...
    struct StoreData : IStoredData<StoreData>
    {
        ...
        public static StoreData BuildDefault(UtilityImpl utilImpl, uint index)
        {
            ...
            result.CoreData = BuildTemplateCoreData(index);
        ...
        // [Nextendo] Build a default Mii (appearance template `index`) with a fresh
        // random CreateId, WITHOUT a UtilityImpl/ITickSource — so it can be created
        // from the UI outside a running game. Device CRC uses the fixed Ryujinx device
        // id, so the result is valid on every Ryujinx install (portable across machines).
        public static StoreData BuildNextendo(uint index)
        {
        ...
        private static CoreData BuildTemplateCoreData(uint index)
        {
