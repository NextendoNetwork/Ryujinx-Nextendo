...
namespace Ryujinx.Horizon.Sdk.Friends.Detail.Ipc
{
    partial class NotificationService : INotificationService, IDisposable
    {
        ...
        public Result GetEvent([CopyHandle] out int eventHandle)
        {
            ...
            // [Nextendo] Splatoon 2's in-game friend screen opens this notification service and then
            // WAITS for a FriendListUpdate signal before it ever calls GetFriendList. The stock
            // emulator never fired that signal (NotificationEventHandler.SignalFriendListUpdate was a
            // TODO), so S2's friend screen froze forever — no list, no back button. Signal it right
            // as the game starts listening so it proceeds to query the (Nextendo) friend list.
            // This runs on the Horizon IPC thread (the game called us), so Os.SignalSystemEvent has
            // its thread context — a background Timer would NOT (NRE), so we hook GetEvent instead.
            // MK8/SSBU call GetFriendList directly and are unaffected.
            SignalFriendListUpdate(_userId);
