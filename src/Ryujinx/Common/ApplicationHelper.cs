...
namespace Ryujinx.Ava.Common
{
    internal static class ApplicationHelper
    {
        ...
        // ---- [Nextendo] cloud-save sync ---------------------------------------
        // Saves are stored PLAINTEXT on the host (DirectorySaveDataFileSystem), with
        // the committed snapshot in "<save>/0". We zip/unzip that directory directly;
        // EnsureApplicationSaveData registers the save in LibHac's index when missing.

        private static bool TryGetUserSaveDataId(ulong titleId, BlitStruct<ApplicationControlProperty> controlHolder, bool create, out ulong saveDataId)
        {
            saveDataId = default;

            if (_horizonClient == null || _accountManager?.LastOpenedUser == null)
            {
                return false;
            }

            Ryujinx.HLE.HOS.Services.Account.Acc.UserId ryuUser = _accountManager.LastOpenedUser.UserId;
            LibHac.Fs.UserId fsUser = new((ulong)ryuUser.High, (ulong)ryuUser.Low);
            SaveDataFilter filter = SaveDataFilter.Make(titleId, default, fsUser, default, default);

            Result rc = _horizonClient.Fs.FindSaveDataWithFilter(out SaveDataInfo info, SaveDataSpaceId.User, in filter);
            if (rc.IsSuccess())
            {
                saveDataId = info.SaveDataId;
                return true;
            }

            if (!create || !ResultFs.TargetNotFound.Includes(rc))
            {
                return false;
            }
            ...
            LibHac.Account.Uid uid = ryuUser.ToLibHacUid();
            rc = _horizonClient.Fs.EnsureApplicationSaveData(out _, new ApplicationId(titleId), in control, in uid);
            if (rc.IsFailure())
            {
                return false;
            }

            rc = _horizonClient.Fs.FindSaveDataWithFilter(out info, SaveDataSpaceId.User, in filter);
            if (rc.IsSuccess())
            {
                saveDataId = info.SaveDataId;
                return true;
        ...
        // [Nextendo] True only when the CURRENTLY-SELECTED Switch profile is the one bound to the
        // Nextendo account. Online + cloud-save-sync must run ONLY on that profile — otherwise a
        // local/other profile (with its own empty save) drives the sync and an empty save gets
        // pushed/pulled, wiping progress. If no profile is bound yet, don't gate (backward compat).
        public static bool IsNextendoProfileActive()
        {
            try
            {
                if (_accountManager?.LastOpenedUser == null)
                {
                    return false;
                }
                ...
                return string.Equals(_accountManager.LastOpenedUser.UserId.ToString(), bound, StringComparison.OrdinalIgnoreCase);
            }
            catch
            {
                return true;
        ...
        // True if the current user already has a non-empty local save for titleId. Used by the
        // cloud PULL to avoid overwriting real local progress with a (possibly older) cloud copy
        // every launch — the cloud is only restored onto a machine that has no local save yet.
        public static bool LocalSaveHasContent(ulong titleId)
        {
            try
            {
                if (!TryGetUserSaveDataId(titleId, new BlitStruct<ApplicationControlProperty>(1), create: false, out ulong saveDataId))
                {
                    return false;
                }

                string root = Path.Combine(VirtualFileSystem.GetNandPath(), $"user/save/{saveDataId:x16}");
                foreach (string sub in new[] { "0", "1" })
                {
            ...
            {
                Logger.Warning?.Print(LogClass.Application, $"[Nextendo] local save check failed: {ex.Message}");
            }

            return false;
        }

        // Export the current user's save for titleId as ZIP bytes, or null if none exists.
        public static byte[] ExportNextendoSave(ulong titleId)
        {
            try
            {
                if (!TryGetUserSaveDataId(titleId, new BlitStruct<ApplicationControlProperty>(1), create: false, out ulong saveDataId))
                {
                    Logger.Info?.Print(LogClass.Application, $"[Nextendo] export {titleId:x16}: NO save data found for the current user");
                    return null;
                }

                string root = Path.Combine(VirtualFileSystem.GetNandPath(), $"user/save/{saveDataId:x16}");
                string src = Path.Combine(root, "0");

                if (!Directory.Exists(src) || Directory.GetFileSystemEntries(src).Length == 0)
                {
                    string work = Path.Combine(root, "1");
                    if (Directory.Exists(work) && Directory.GetFileSystemEntries(work).Length > 0)
                    {
                        src = work;
                    }
                    else
                    {
                        Logger.Info?.Print(LogClass.Application, $"[Nextendo] export {titleId:x16}: save {saveDataId:x16} committed folder EMPTY ({root})");
                        return null; // nothing saved yet
                    }
                }

                string tmpZip = Path.Combine(Path.GetTempPath(), $"nxsave_{saveDataId:x16}_{Environment.TickCount}.zip");
                if (File.Exists(tmpZip))
                {
                    File.Delete(tmpZip);
                }

                System.IO.Compression.ZipFile.CreateFromDirectory(src, tmpZip);
                byte[] bytes = File.ReadAllBytes(tmpZip);
                File.Delete(tmpZip);

                Logger.Info?.Print(LogClass.Application, $"[Nextendo] export {titleId:x16}: save {saveDataId:x16} -> {bytes.Length} B (from {src})");
                return bytes;
            }
            catch (Exception ex)
            {
                Logger.Warning?.Print(LogClass.Application, $"[Nextendo] save export failed: {ex.Message}");
                return null;
            }
        }

        // Import ZIP bytes into the current user's save for titleId (overwrites the
        // committed snapshot). Creates the save data first if it doesn't exist.
        public static bool ImportNextendoSave(ulong titleId, BlitStruct<ApplicationControlProperty> controlHolder, byte[] zipBytes)
        {
            ...
            try
            {
                if (!TryGetUserSaveDataId(titleId, controlHolder, create: true, out ulong saveDataId))
                {
                    return false;
                }

                string root = Path.Combine(VirtualFileSystem.GetNandPath(), $"user/save/{saveDataId:x16}");
                string target = Path.Combine(root, "0");
                ...
                string tmpZip = Path.Combine(Path.GetTempPath(), $"nxsave_in_{saveDataId:x16}_{Environment.TickCount}.zip");
                File.WriteAllBytes(tmpZip, zipBytes);
                System.IO.Compression.ZipFile.ExtractToDirectory(tmpZip, target, overwriteFiles: true);
                File.Delete(tmpZip);
            ...
            {
                Logger.Warning?.Print(LogClass.Application, $"[Nextendo] save import failed: {ex.Message}");
                return false;
            }
        }
    }
